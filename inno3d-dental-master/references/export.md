# 수출·물류 참조 (Export)

## 기본 수출 조건
- 인코텀즈: EX-WORKS Incheon (현재 기준)
- 통화: USD
- HS코드: 9018.49.9000(스캐너) / 8523.49.9000(SW) / 9018.90.9900(악세서리)
- 발행: Commercial Invoice, Packing List, 필요시 Certificate of Origin

## 국가별 주의사항
| 국가 | 주요 리스크 | 확인 필요 |
|------|-----------|---------|
| 이란 | 국제제재·결제경로 | IRC코드·USD결제경로법무검토 |
| 러시아 | DHL 서비스 2022 중단 | 대체 배송사 확인 |
| 터키 | ÜTS 등록 | IFU·DoC·EC REP·GS1 GTIN |
| 일본 | PMDA Class II | 현지딜러경유 여부 |

## 인보이스 템플릿
[확인 필요] `Commercial_Invoice_Intraoral_Scanner.docx` 는 이 스킬 폴더에 동봉되어 있지 않다.
담당자 로컬 또는 사내 공유 드라이브에 있으므로, 파일이 필요하면 사용자에게 업로드를 요청한다.
스킬이 이 파일을 자동으로 열 수 있다고 가정하지 않는다.
